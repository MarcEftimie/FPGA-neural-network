...
f = open(f"{project_name}/create_testbench_templates.py", "w")
f.write("""import re
import os
...
f = open(f"{project_name}/Nexys-4-Master.xdc", "w")
f.write("""## This file is a general .xdc for the Nexys4 DDR Rev. C
...
f = open(f"{project_name}/build.tcl", "w")
f.write(f"""# Xilinx Vivado Gui-less build script
...
synth_design -top {main_file_name} -part xc7a100tcsg324-1 -directive runtimeoptimized
...
f = open(f"{project_name}/program.tcl", "w")
...
f = open(f"{project_name}/.gitignore", "w")
